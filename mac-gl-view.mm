...
{
    OpenGLView* self_ = (__bridge OpenGLView*)displayLinkContext;

    [self_.openGLContext makeCurrentContext];

    CGLLockContext([self_.openGLContext CGLContextObj]);
    ...
    CGLUnlockContext([self_.openGLContext CGLContextObj]);
...
- (void) initGL
{
	...
	_renderer = [self createRendererWithDefaultFBO:0];
}
